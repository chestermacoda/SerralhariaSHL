...
if(isset($_POST['registo'])){
    ...
    // $date = date('Y-m-d') ;
    $date = date('Y-m-d') ;
    $cmd = $pdo->prepare("SELECT en.id,m.NomeMaterial,m.tipo,m.medida,en.quantidade,en.data FROM `entrada` en INNER join material m ON en.id_material = m.id where en.data = '$date'");
    $cmd->execute();
    $dados = $cmd->fetchAll();
    if($cmd->rowCount() > 0){
        foreach($dados as $k ){
